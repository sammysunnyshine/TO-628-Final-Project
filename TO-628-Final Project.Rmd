...
# creating dummy variable to distinguish popular and less well-known artists
spot2$artist_popularity <- 0
spot2$artist_popularity[spot2$artist %in% criteria_vec] <- 1

# Deleting artist - too many unique values
spot2$artist <- NULL

#Try running without popular artists included
#spot2 <- spot2[spot2$artist_popularity == 0, ]
#spot2$artist_popularity <-NULL
...
## Randomize the Data Set
```{r}
spot2_random <- spot2[sample(1:nrow(spot2)), ]
```
...
spot2_norm <- as.data.frame(lapply(spot2_random, normalize))
```

## Split Test and Train
```{r}
set.seed(12345)
test_set <- sample(1:nrow(spot2_norm), 0.5*nrow(spot2_norm))
spot_test <- spot2_norm[test_set, ]
spot_train <- spot2_norm[-test_set, ]
